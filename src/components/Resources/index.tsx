...
import { ITEMS, RESOURCES, CATEGORIES, EXPANDED_RESOURCE_NAMES } from "../../constant/resources";
    ...
    <div className="block">
      ...
      <div className="input-group">
        <span className="pointer-events-none select-none border-drg-primary-700 border">
          <img src={`assets/${item.toLowerCase()}.webp`} className="w-6 max-w-none" />
        </span>
        <input
          id={name.toLowerCase()}
          type="text"
  ...
  return (
    <div className="w-full ">
      {Object.entries(CATEGORIES).map(([category, items]) => {
        ...
        return (<div className="not-first:mt-10" key={category}>
          ...
          <div className="mt-3 md:w-auto grid grid-cols grid-rows-1 gap-2 md:grid-cols-2 lg:grid-cols-2 lg:gap-x-10 xl:grid-cols-3">
            ...
            {items.map((item: ITEMS) => (
              <Resource key={item} item={item} name={EXPANDED_RESOURCE_NAMES[item] ?? item} uuid={RESOURCES[item]} />
            ))}
          </div>
        </div>)
      })}
